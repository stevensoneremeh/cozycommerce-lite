      ...
      <div className="w-full px-4 mx-auto max-w-7xl sm:px-8 xl:px-0">
        <div className="relative overflow-hidden z-1 rounded-xl bg-gray-2 p-4 sm:p-7.5 lg:p-10 xl:p-15">
          <div className="max-w-[422px] w-full">
            <span className="block font-medium lg text-blue mb-2.5">{data.subtitle}</span>

            <h2 className="mb-3 text-xl font-semibold text-dark lg:text-heading-4 xl:text-heading-3">{data.title}</h2>
            <p className="text-base font-normal text-dark-3">{data?.product?.title}</p>
...
const BackgroundImages = ({ data }: CountdownBannerProps) => (
  <>
    ...
    />
    <Image
      src={data.countdownImage || "/placeholder.svg"}
      alt="product"
